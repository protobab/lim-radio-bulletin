...
async function generatePlaylist() {
    ...
    console.log(`🔎 Building 100-song playlist: ${fileName}...`);
    ...
    for (const q of queries) {
        ...
        try {
            const res = await fetch(`https://archive.org/advancedsearch.php?q=${encodeURIComponent(q)} AND mediatype:(audio)&fl[]=identifier&rows=60&output=json`);
            const data = await res.json();
            const items = data.response?.docs || [];

            for (const item of items) {
                if (foundCount >= 100) break;
                // Generate the direct streaming link
                const link = `https://archive.org/download/${item.identifier}/${item.identifier}_vbr.mp3`;
                playlistContent += `#EXTINF:-1,${item.identifier}\n${link}\n`;
                foundCount++;
            }
        } catch (e) { console.log(`⚠️ Query skipped: ${e.message}`); }
    }
    ...
    if (foundCount > 0) {
        ...
        console.error("❌ No music found. Verify your queries.");
        process.exit(1);
    }
}
